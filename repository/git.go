...
// noHooks disables git hooks for a single git invocation
// (git -c core.hooksPath=/dev/null ...). container-use's own git
// operations (commits, pushes, merges) must never trigger the user
// repository's hooks.
const noHooks = "core.hooksPath=/dev/null"
