...
Container Use provides multiple layers of secret protection:

### 🔒 **AI Model Isolation**
- The AI model never sees actual secret values
- Secrets are resolved within the container, not in the chat context
- Even if an agent tries to output secrets, only references are visible

### 📝 **Log Protection** 
- Secret values are automatically stripped from command outputs
- Development logs remain safe to share and review
- Prevents accidental leaks through echo, cat, or debugging commands

### 🔐 **Container Isolation**
- Secrets are only available within the specific environment
- Each environment gets fresh secret resolution
- No cross-environment secret leakage

### 🚫 **Limited Agent Access**
- Agents can use secrets through your application code
- Direct secret extraction (like `echo $SECRET`) is blocked from logs
- Maintains usability while preventing easy harvesting

## Next Steps

<CardGroup cols={2}>
  <Card title="Environment Configuration" icon="gear" href="/environment-configuration">
    Learn about other environment configuration options
  </Card>
  <Card title="Environment Workflow" icon="arrows-rotate" href="/environment-workflow">
    Understand how to work with agent environments
  </Card>
</CardGroup>