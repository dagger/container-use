...
claude mcp add container-use -- <full path to container-use command> stdio
```
...
Then configure the Container Use MCP server in the Zed `settings.json`. Provide the absolute path to the `container-use` executable:
...
"context_servers": {
  "container-use": {
    "source": "custom",
    "command": {
      "path": "/opt/homebrew/bin/container-use",
      "args": ["stdio"],
      "env": {}
    }
  }
...
> Note: Provide the absolute path to your systems `container-use` executable:
...
```json
{
  ...
  "mcp": {
    "container-use": {
      "type": "local",
      "command": ["/path/to/container-use", "stdio"],
      "enabled": true
    }
  }
}
...
```json
{
  "mcpServers": {
    "container-use": {
      "command": "replace with pathname of container-use",
      "args": ["stdio"],
      "env": {},
      "alwaysAllow": [],
      "disabled": false
